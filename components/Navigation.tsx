...
export default function Navigation() {
  ...
  return (
    <>
        ...
        {navGroups.map((group) => (
          ...
          >
            <button
              ...
              className={`px-3 py-2 rounded-xl text-xs md:text-sm font-medium transition-all flex items-center space-x-1.5 ${
                isGroupActive(group)
                  ? 'bg-strava text-white shadow-soft'
            ...
            {openDropdown === group.label && (
              <div 
                className="absolute top-full left-0 mt-2 w-52 glass py-2 z-50 animate-slide-up"
              ...
              >
                {group.items.map((item) => (
                  <Link
                    ...
                    className={`flex items-center space-x-3 px-4 py-2.5 text-sm transition-colors mx-1 rounded-xl ${
                      isActive(item.href)
                        ? 'bg-strava/10 text-strava dark:bg-strava/20 dark:text-strava-muted font-medium'
      ...
      {/* Mobile Menu */}
      {mobileMenuOpen && (
        <div className="md:hidden fixed top-16 left-0 right-0 bg-white/85 dark:bg-slate-900/85 backdrop-blur-2xl border-t border-white/20 dark:border-white/5 z-50 max-h-[calc(100vh-4rem)] overflow-y-auto animate-slide-up">
